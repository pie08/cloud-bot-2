...
class Util(commands.Cog):
    ...
    async def serverinfo(self, ctx):
        ...
        em.add_field(name = '*Creation Date*', value = creation_date.strftime("%m/%d/%Y"), inline = False)
        ...
        em.add_field(name = '*Voice Channels*', value = str(voicecha), inline = False)
        em.add_field(name = '*Welc status*', value = f'{welc_status}', inline = False)
    ...
    async def warns(self, ctx, member: nextcord.Member=None):
        ...
        with open('C:\\Users\\Tyrus\\OneDrive\\Desktop\\peepo bot\\warns.json') as f:
            data = json.load(f)
            for d in data:
                if d['user_id'] == member.id:
                    warns = d['warns']
                    date = d['warn_date_remove'][:11]
                    em=nextcord.Embed(description=f'*Warnings expire -* `{date}`')
                    em.set_author(name=f'{member} currently has {warns} warnings', icon_url=member.avatar.url)
    ...
    async def whois(self, ctx, member : nextcord.Member):
        ...
        embed = nextcord.Embed(description = member.mention , color = nextcord.Color.blue())
        embed.set_author(name = member.name, icon_url = member.avatar.url)
        embed.set_thumbnail(url = member.avatar.url)
        embed.set_footer(icon_url = ctx.author.avatar.url, text = f'ID: ' + str(member.id))
        embed.add_field(name = '🔓 ID', value=f'`{member.id}`', inline=False)
        embed.add_field(name = '❌ Discriminator', value=f'`{member.discriminator}`', inline=False)
        embed.add_field(name = '⚡ Highest Role', value = f'{roles[-1]}', inline=False)
        embed.add_field(name = '🕔 Joined At', value = join_date.strftime("%m/%d/%Y"), inline = False)
        embed.add_field(name = '🕔 Account Created At', value = creation_date.strftime("%m/%d/%Y"), inline=False)
