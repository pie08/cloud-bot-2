...
class Commands(commands.Cog):
    ...
    async def ping(self, ctx):
        em = nextcord.Embed(title = 'Pong 🏓', description = '⌛**Time** ' + f'{round(self.client.latency * 1000)}ms', color = nextcord.Color.blue())
        em.timestamp = datetime.datetime.utcnow()
        await ctx.send(embed=em)

    @commands.command()
    async def fiverr(self, ctx):
        em=nextcord.Embed(title='Dev Fiverr Link', description=f"With this link you can order a custom discord bot from {self.client.user.name}'s dev on fiverr, [Fiverr Link](https://www.fiverr.com/tyrus_b/program-a-professional-and-custom-discord-bot-for-you) \n[Fiverr Link 2](https://www.fiverr.com/share/3V5Y19)", color=nextcord.Color.green())
        em.set_thumbnail(url='https://cdn-images-1.medium.com/max/1200/1*FfEOiku2EkgoQy_bf8UL6A.png')
    ...
    async def info(self, ctx):
        ...
        em=nextcord.Embed(description=f'**Guild prefix :** `{prefix}`', color=nextcord.Color.blue())
        ...
        em.add_field(name='🔗 Useful Links', value='[My Fiverr](https://www.fiverr.com/tyrus_b/program-a-professional-and-custom-discord-bot-for-you) | [Support Server](https://nextcord.gg/72udgVqEkf) | [Invite Me](https://top.gg/bot/881336046778986518)', inline=False)
        em.set_footer(icon_url=ctx.author.avatar.url, text=f'Requested by {ctx.author.name}')
    ...
    async def feedback(self, ctx):
        ...
        try:
            em=nextcord.Embed(title = f'Welcome to the feedback section of {self.client.user.name}', description = 'Here you can submit feedback such as what you like about me, what you dont like about me, what you think can be improved, suggest new features to be added to me etc. Please enter your response now, you have 5 minutes', color=nextcord.Color.blue())
        ...
        if len(str(msg.content)) < 15:
            ...
            return
        em=nextcord.Embed(description=f'Feedback submitted by {ctx.author.mention}', color=nextcord.Color.red())
        em.set_author(icon_url=ctx.author.avatar.url, name='Suggestion Submitted')
        em.add_field(name='Feedback Description', value=f'*{str(msg.content)}*')
        em.set_thumbnail(url = self.client.user.avatar.url)
        em.timestamp = datetime.datetime.utcnow()
    ...
    async def support(self, ctx):
        em=nextcord.Embed(description='Heres my support server! \nhttps://nextcord.gg/72udgVqEkf', color=nextcord.Color.blue())
        await ctx.author.send(embed=em)

    @commands.command()
    async def invite(self, ctx):
        user = ctx.author
        em=nextcord.Embed(description='https://top.gg/bot/881336046778986518', color=nextcord.Color.blue())
